	...
	else
	{
		if ($category_result->RecordCount() > 0)
		{
			...
			while ($show_category = $category_result->FetchRow())
			{
				...
				$this->body .= str_replace(">", " class=navigation_1_name>", $link).urldecode(stripslashes($show_category['category_name']));
				$this->body .= "</a>";
				if ($show_module['display_category_count'])
				{
					$css = array("listing_count" => "navigation_1_count", "auction_count" => "navigation_1_auction_count", "ad_count" => "navigation_1_ad_count");
				...
				if ($show_module['display_category_description'])
					$this->body .="</a><br><font class=navigation_1_description>".urldecode(stripslashes($show_category['description']))."</font>";
				$this->body .="</td>";
