...
{
	$this->sql_query = "select module_logged_in_html from ".$this->pages_table." where page_id = 192";
...
{
	$this->sql_query = "select module_logged_out_html from ".$this->pages_table." where page_id = 192";
	$logged_result = $db->Execute($this->sql_query);
	if (!$logged_result)
	{
		return false;
