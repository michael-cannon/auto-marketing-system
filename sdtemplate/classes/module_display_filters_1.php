...
{
	echo "<br>TOP OF MODULE_DISPLAY_FILTERS_1.PHP<bR>\n";
	echo $this->configuration_data->USE_FILTERS." is use filters in display filter module 1<br>\n";
...
if ($this->configuration_data['use_filters'])
{
	...
	$dropdown_labels[1] = urldecode($this->messages[1482]);
	$dropdown_labels[2] = urldecode($this->messages[1483]);
	$dropdown_labels[3] = urldecode($this->messages[1484]);
	$dropdown_labels[4] = urldecode($this->messages[1485]);
	$dropdown_labels[5] = urldecode($this->messages[1486]);
	$dropdown_labels[6] = urldecode($this->messages[1487]);
	$dropdown_labels[7] = urldecode($this->messages[1488]);
	$dropdown_labels[8] = urldecode($this->messages[1489]);
	$dropdown_labels[9] = urldecode($this->messages[1490]);
	$dropdown_labels[10] = urldecode($this->messages[1491]);
	$select_previous[1] = urldecode($this->messages[1531]);
	$select_previous[2] = urldecode($this->messages[1532]);
	$select_previous[3] = urldecode($this->messages[1533]);
	$select_previous[4] = urldecode($this->messages[1534]);
	$select_previous[5] = urldecode($this->messages[1535]);
	$select_previous[6] = urldecode($this->messages[1536]);
	$select_previous[7] = urldecode($this->messages[1537]);
	$select_previous[8] = urldecode($this->messages[1538]);
	$select_previous[9] = urldecode($this->messages[1539]);
	$select_previous[10] = urldecode($this->messages[1540]);
	$clear_labels[1] = urldecode($this->messages[1492]);
	$clear_labels[2] = urldecode($this->messages[1493]);
	$clear_labels[3] = urldecode($this->messages[1494]);
	$clear_labels[4] = urldecode($this->messages[1495]);
	$clear_labels[5] = urldecode($this->messages[1496]);
	$clear_labels[6] = urldecode($this->messages[1497]);
	$clear_labels[7] = urldecode($this->messages[1498]);
	$clear_labels[8] = urldecode($this->messages[1499]);
	$clear_labels[9] = urldecode($this->messages[1500]);
	$clear_labels[10] = urldecode($this->messages[1501]);
	$last_one = 0;
	$this->body = "<form name=filter_display_module_form action=".$this->configuration_data['classifieds_file_name']."?".$_SERVER["QUERY_STRING"]." method=post>\n";
	...
	if ($this->filter_id)
	{
		...
		if ($current_filter["filter_level"] != 1)
		{
			...
			do
			{
				...
				{
					$this->body .= "<td class=filter_dropdown_1><select class=filter_dropdown_1 name=set_filter_id[".$parents_level."] onChange=\"if(this.options[this.selectedIndex].value != '') this.form.submit();\">\n\t";
		...
		elseif ($filter_result->RecordCount() > 0)
		{
			...
			if (!$show_module['module_display_filter_in_row'])
				$this->body .= "<tr>";
			$this->body .= "<td class=filter_dropdown_1><select class=filter_dropdown_1 name=set_filter_id[".$level."] onChange=\"if(this.options[this.selectedIndex].value != '') this.form.submit();\">\n\t\t";
			...
			while ($show_filter = $filter_result->FetchRow())
			{
				...
				//else
				{
					...
					//if (($show_filter["filter_level"] == ($current_filter["filter_level"] + 1)) && ($filter_result->RecordCount() > $total_options))
					{
						...
						if ($show_module['module_display_filter_in_row'])
						{
							$this->body .= "</td>\n\t<td class=filter_dropdown_1>\n\t";
							$this->body .= "<select name=set_filter_id[".$level."] class=filter_dropdown_1 onChange=\"if(this.options[this.selectedIndex].value != '') this.form.submit();\">\n\t";
						}
						else
						{
							$this->body .= "</td>\n</tr>\n<tr>\n\t<td class=filter_dropdown_1>\n\t";
	...
	else
	{
		...
		elseif ($filter_result->RecordCount() > 0)
		{
			...
			$level = 1;
			$this->body .= "<tr><td class=filter_dropdown_1><select class=filter_dropdown_1 name=set_filter_id[".$level."] onChange=\"if(this.options[this.selectedIndex].value != '') this.form.submit();\">\n\t\t";
			$this->body .= "<option value=\"\">".$dropdown_labels[$level]."</option>\n\t\t";
