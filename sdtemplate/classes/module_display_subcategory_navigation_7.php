...
if (!$category_result)
{
	$this->error_message = "<font class=error_message>".urldecode($this->messages[2431])."</font>";
	return false;
}
else
{
	if ($category_result->RecordCount() > 0)
	{
		...
		while ($show_category = $category_result->FetchRow())
		{
			...
			{
				$this->body .= str_replace(">", " class=navigation_subcategory_7_name>", $link)."<img src=\"".$show_category['category_image']."\" hspace=2 vspace=0 border=0 align=left>";
			}
			//$category_name = $this->get_category_name($db,$show_category['category_id']);
			$this->body .= str_replace(">", " class=navigation_subcategory_7_name>", $link).urldecode(stripslashes($show_category['category_name']));
			if ($show_module['module_display_new_ad_icon'])
					$this->check_category_new_ad_icon_use($db,$show_category['category_id']);
			$this->body .= "</a>";
			if ($show_module['display_category_count'])
			{
				$css = array("listing_count" => "navigation_subcategory_7_count", "auction_count" => "navigation_subcategory_7_auction_count", "ad_count" => "navigation_subcategory_7_ad_count");
			...
			if ($show_module['display_category_description'])
				$this->body .="<br><font class=navigation_subcategory_7_description>".urldecode(stripslashes($show_category['description']))."</font>";
			$this->body .="</td>";
			if ($show_module['number_of_browsing_columns'] > 1)
			{
				if ($show_category = $category_result->FetchRow())
				{
					...
					if ($show_module['display_category_description'])
						$this->body .="</a><br><font class=navigation_subcategory_7_description>".urldecode(stripslashes($show_category['description']))."</font>";
					$this->body .="</td>";
				}
				else
				{
		...
		if (($show_module['module_display_ad_description']) && ($current_category))
		{
			...
			$this->body .="<tr class=navigation_subcategory_7_subcategories_to>\n\t<td>\n\t".urldecode($this->messages[2433])." ".$parent_category_name->CATEGORY_NAME."\n\t</td>\n</tr>\n";
		}
		$this->body .="</table>";
	}
	else
	{
		...
		if ($show_module['display_no_subcategory_message'])
			$this->body .="<tr class=navigation_subcategory_7_subcategories_to>\n\t<td>\n\t".urldecode($this->messages[2432])." ".$parent_category_name->CATEGORY_NAME."\n\t</td>\n</tr>\n";
	}
}
?>
