...
else
{
	...
	if (!$category_result)
	{
		$this->error_message = "<font class=error_message>".urldecode($this->messages[1519])."</font>";
		return false;
	}
	else
	{
		if ($category_result->RecordCount() > 0)
		{
			...
			while ($show_category = $category_result->FetchRow())
			{
				...
				$this->body .= str_replace(">", " class=navigation_3_name>", $link).urldecode(stripslashes($show_category['category_name']));
				...
				if ($show_module['display_category_description'])
					$this->body .="</a><br><font class=navigation_3_description>".urldecode(stripslashes($show_category['description']))."</font>";
				$this->body .="</td>";
				...
				if (($show_module['module_display_ad_description']) && ($this->site_category))
					$this->body .="<tr class=navigation_3_subcategories_to>\n\t<td>\n\t".urldecode($this->messages[1892])." ".$current_category_name."\n\t</td>\n</tr>\n";
		...
		else
		{
			...
			if ($show_module['display_no_subcategory_message'])
				$this->body .="<tr class=navigation_3_subcategories_to>\n\t<td>\n\t".urldecode($this->messages[1520])." ".$current_category_name."\n\t</td>\n</tr>\n";
		}
