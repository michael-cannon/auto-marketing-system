...
elseif ($featured_result->RecordCount() > 0)
{
	...
	if ($featured_result->RecordCount() > 0)
	{
		...
		if ($show_module['module_display_header_row'])
			$this->body .="<tr class=featured_ad_pics_3_header_row><td colspan=".$show_module['module_number_of_columns'].">
				".urldecode($this->messages[1598])."</td></tr>\n\t";
		...
		while ($show = $featured_result->FetchRow())
		{
			if ($show_module['module_number_of_ads_to_display'] > $row)
			{
				...
				if($this->is_class_auctions())
				{
					...
					{
						$this->body .= "<br>";
						$this->body .= urldecode($this->messages[200031]);
					...
					{
						$this->body .= "<br>";
						$this->body .= urldecode($this->messages[200032]);
