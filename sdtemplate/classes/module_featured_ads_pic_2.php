...
$this->sql_query = "select * from geodesic_classifieds,".$this->images_urls_table." where
	geodesic_classifieds.id = ".$this->images_urls_table.".classified_id and ".$this->images_urls_table.".display_order = 1 and
	featured_ad = 1 and image > 0 and live = 1 ".$this->sql_zip_filter_in_statement.$this->sql_state_filter_statement.$sql_filter_in_statement.$type_in_statement;
...
elseif ($featured_result->RecordCount() > 0)
{
	...
	if ($featured_result->RecordCount() > 0)
	{
		...
		if ($show_module['module_display_header_row'])
			$this->body .="<tr class=featured_ad_pics_2_header_row><td colspan=".$show_module['module_number_of_columns'].">
				".urldecode($this->messages[1472])."</td></tr>\n\t";
		...
		while ($show = $featured_result->FetchRow())
		{
			...
			{
				$this->body .="<tr class=featured_ad_pics_2_pic_row><td width=".$column_width." valign=top>";
				...
				$this->featured_ads_pic_level_1_display($db, $show, $show_module, "featured_ad_pics_2_pic_row");
				...
				if($this->is_class_auctions())
				{
					...
					{
						$this->body .= "<br>";
						$this->body .= urldecode($this->messages[200029]);
					...
					{
						$this->body .= "<br>";
						$this->body .= urldecode($this->messages[200030]);
...
{
	$this->body .="<table cellpadding=3 cellspacing=1 border=0 align=center width=100%><tr><td class=featured_ad_pics_2_pic_row>".urldecode($show_module['display_empty_message'])."</td></tr></table>\n";
}
?>