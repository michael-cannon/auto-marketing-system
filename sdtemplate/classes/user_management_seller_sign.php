<? //user_management_seller_sign.php
/**************************************************************************\
...
class Display_seller_sign extends Site {
		...
		if ($id)
		{
			...
			if ($show)
			{
				if ($show->LIVE == 1)
				{
					...
					$this->sql_query = "select template_id from ".$this->pages_templates_table." where language_id = ".$this->language_id." and page_id = 71";
...
//#################################################################################
	...
	function get_ads_extra_checkboxs($db,$classified_id)
	{
		...
		elseif ($special_result->RecordCount() > 0 )
		{
			...
			while ($show_special = $special_result->FetchNextObject())
			{
				$current_line = str_replace("<<EXTRA_CHECKBOX_NAME>>",urldecode($show_special->NAME),$extra_template);
				$question_block .= $current_line;
			}
			//$this->body .="</table>\n\t</td>\n</tr>\n";
		}
